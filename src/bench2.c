...
static int benchmark(void *memory, size_t memory_size, uint32_t t_cost,
                     uint32_t m_cost, uint32_t p) {
  ...
  printf("%8lu%16lu%8lu%16.6lf%16.6lf%16.6lf\n", (unsigned long)t_cost,
         (unsigned long)m_cost, (unsigned long)p, ms_d_final, ms_i_final,
         ms_id_final);
  return 0;
}
