...
static void run(uint32_t outlen, char *pwd, char *salt, uint32_t t_cost,
  ...
  encodedlen =
      argon2_encodedlen(t_cost, m_cost, lanes, (uint32_t)saltlen, outlen, type);
  ...
  result = argon2_hash(t_cost, m_cost, threads, pwd, pwdlen, salt, saltlen, out,
...
int main(int argc, char *argv[]) {
  ...
  for (i = 2; i < argc; i++) {
    ...
    if (!strcmp(a, "-h")) {
      ...
      if (i < argc - 1) {
        i++;
        input = strtoul(argv[i], NULL, 10);
        if (input == 0 || input == ULONG_MAX || input > ARGON2_MAX_THREADS ||
  ...
  run(outlen, pwd, salt, t_cost, m_cost, lanes, threads, type, encoded_only,
      raw_only, version);
